...
// `let`, not `const` — a bad explicit DATA_DIR (see ensureDataDirUsable
// below) falls back to DEFAULT_DATA_DIR at runtime, reassigning this.
let DATA_DIR = process.env.DATA_DIR ? path.resolve(process.env.DATA_DIR) : DEFAULT_DATA_DIR;
// Set true if an explicit DATA_DIR was rejected (see seedDataDirIfNeeded
// below) and DATA_DIR was overridden back to DEFAULT_DATA_DIR — lets
// server.js's startup log say what actually happened instead of just
// echoing whether process.env.DATA_DIR was originally set.
let DATA_DIR_FELL_BACK = false;

// Creates `dir` if needed and proves it's actually writable (not just that
// mkdir didn't throw) by writing and removing a small probe file. Throws
// whatever fs error it hit if not.
function ensureDataDirUsable(dir) {
...
function seedDataDirIfNeeded() {
  try {
    ensureDataDirUsable(DATA_DIR);
  } catch (err) {
    // Fixed 2026-09-08 (ask: "Render project is not able to deploy" — an
    // EACCES crash on mkdir '/var/data'): an explicit DATA_DIR pointing at
    // a path with no real disk behind it — e.g. left over from a
    // render.yaml/Blueprint attempt where the disk itself was never
    // actually provisioned, which is exactly what happens on Render's free
    // tier, since it doesn't support persistent disks at all — used to
    // crash the whole app at startup, every single deploy, with no way to
    // recover except fixing the environment variable blind (the process
    // never got far enough to log anything useful about why). Now: only an
    // *explicit* DATA_DIR gets this safety net (the bare default has
    // nowhere sensible left to fall back to, so a failure there still
    // throws) — log a loud, specific warning and fall back to the default
    // location instead of taking the whole app down over one bad env var.
    if (!process.env.DATA_DIR) throw err;
    console.error(
      'WARNING: DATA_DIR is set to "' + DATA_DIR + '" but it could not be ' +
      'created or written to (' + (err.code || err.message) + '). This ' +
      'usually means DATA_DIR points at a path with no real persistent ' +
      'disk behind it — e.g. a Blueprint/render.yaml deploy where the ' +
      'disk itself was never provisioned (this always happens on Render\'s ' +
      'free tier, which has no persistent disk option at all). Falling ' +
      'back to "' + DEFAULT_DATA_DIR + '" instead of crashing, but this ' +
      'means data is NOT going to whatever storage DATA_DIR was meant to ' +
      'point at. Fix: remove or correct the DATA_DIR environment variable ' +
      '(Render dashboard → this service → Environment tab) and redeploy.'
    );
    DATA_DIR = DEFAULT_DATA_DIR;
    DATA_DIR_FELL_BACK = true;
    ensureDataDirUsable(DATA_DIR); // let this one throw for real if it also fails
  }
...
module.exports = { readAll, writeAll, upsert, upsertMany, upsertManyBy, remove, normId, readObject, writeObject, DATA_DIR, DATA_DIR_FELL_BACK };
