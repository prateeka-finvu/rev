...
const SCENARIO_DEFINITIONS = [
  {
    ...
    label: 'Lending DF volume falls 50% (not ~34%) during SUC Cliff',
    description: 'During the SUC Cliff (the first 3 months of the SUC period), every Lending FIU whose own SUC Cliff CMGR is already negative has that Cliff CMGR replaced with a flat -20.63%/month — the constant monthly rate that compounds to exactly a 50% cumulative fall over 3 months, versus the roughly 34% fall a typical -13%/month Cliff CMGR produces on its own. A Lending FIU whose Cliff CMGR is zero or positive is left untouched (there is no sensible "worse" reading for a FIU that isn’t already degrading). SUC Recovery is left untouched either way: recovery is still assumed to happen at the originally-anticipated pace, not accelerated.'
  },
  {
    key: 'nonBankPfmZero',
