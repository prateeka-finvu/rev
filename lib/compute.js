...
const SCENARIO_DEFINITIONS = [
  {
    ...
    label: 'Lending DF volume falls 50% (not ~34%) during DPI Pricing Cliff',
    description: 'During the DPI Pricing Cliff (the first 3 months of the DPI Pricing period), every Lending FIU whose own DPI Pricing Cliff CMGR is already negative has that Cliff CMGR replaced with a flat -20.63%/month — the constant monthly rate that compounds to exactly a 50% cumulative fall over 3 months, versus the roughly 34% fall a typical -13%/month Cliff CMGR produces on its own. A Lending FIU whose Cliff CMGR is zero or positive is left untouched (there is no sensible "worse" reading for a FIU that isn’t already degrading). DPI Pricing Recovery is left untouched either way: recovery is still assumed to happen at the originally-anticipated pace, not accelerated.'
  },
  {
    key: 'nonBankPfmZero',
    label: 'Non-bank PFM FIUs → ₹0 post-DPI Pricing',
    description: 'Once DPI Pricing kicks in, PFM-use-case FIUs whose License Type isn’t Bank contribute ₹0 revenue for every DPI Pricing month (their DF/usage figures are still shown as normal).'
  }
];
...
// The "Lending CMGR worse during SUC Cliff" scenario (ask: 2026-09-03,
// replacing the original 1.5x-the-rate approach from 2026-09-02): rather
// than scaling each FIU's own Cliff CMGR by a multiplier — which for the
// typical -13%/month baseline gave a 47.83% fall over 3 months, not the
// intended 50% — every eligible FIU's Cliff-phase monthly rate is replaced
// outright with the single constant rate that compounds to exactly a 50%
// cumulative fall over any 3-month window: (1+r)^3 = 0.5, so
// r = 0.5^(1/3) - 1 ≈ -20.63%/month. This is the same target rate for every
// eligible FIU regardless of its own configured Cliff CMGR (a FIU's
// starting dfUsage differs, but compounding at a fixed rate always lands
// each one 50% below its own starting point after 3 months — there's no
// meaningful per-FIU "scaling" beyond that). Only applied to Lending FIUs
// whose own Cliff CMGR is already negative — see LENDING_CMGR_WORSE_ELIGIBLE
// check at the call site.
const LENDING_CMGR_WORSE_RATE = Math.pow(0.5, 1 / 3) - 1; // ≈ -0.2063
...
function applyFixedRevenueOverride(key, monthCols, monthly, sucStartDate) {
  ...
  return monthly.map((m, i) => {
    // Deliberately unconditional — this always overrides the natural
    // computation (and any recorded Historical Actual) for these two FIUs,
    // for every month the schedule/window covers. Confirmed 2026-09-03:
    // this is intentional, not a bug — revisit only on explicit request.
...
function computeRevenue(counts, metadataById, yieldCmgrById, asOfDate, fyStartMonth, sucStartDate, historicalByKey, scenarios) {
  ...
  const duplicateCounts = []; // FIU IDs with more than one row in the upload — only the first is used
  const seenKeys = new Set();

  // Builds one FIU's full row — every month's usage/revenue plus its
  // summary fields — from its config (meta/yc) and this month's counts
  // upload row `c`. Shared by both passes below: FIUs present in the
  // current counts upload, and (further down) FIUs that have FIU Metadata
  // config but no row in this month's upload at all — for the latter, `c`
  // is a blank stand-in ({ activeUsers: '', dataFetches: '' }) so baseline
  // usage comes back NaN and live-projection months correctly show "no
  // data" instead of a bogus zero-based projection, while past/current
  // months can still surface a recorded Historical Actual exactly like any
  // other row (see the call site below for why this matters).
  function buildFiuRow(fiuId, key, meta, yc, c) {
    ...
    // drives revenue.
    //
    // usage/dfUsage below are always what's *displayed* for a given month —
    // the current month's own row always reflects the live counts upload,
    // exactly as before. What future months compound *forward* from is
    // tracked separately, in usageForwardChain/dfForwardChain (fixed
    // 2026-09-09 — ask: "Projected" chart dipping sharply right after the
    // last actual month, then only gradually climbing back over the rest of
    // the FY). The current month's counts upload is very often a
    // month-to-date pull taken partway through the month, not a finished
    // total — Data Fetch counts get extrapolated to a full month for it
    // above (day-of-month ÷ days-in-month), but Active/Unique User counts
    // never do (a partial user count doesn't scale linearly with days
    // elapsed the way DF call volume roughly does), so a mid-month AU count
    // is used as-is. Before this fix, that same possibly-partial figure was
    // also what every later month compounded forward from — permanently
    // understating the whole rest of the FY off one still-accumulating
    // month, not just the current one, and only ever looking like it
    // "recovered" by month's end because CMGR growth from a low base still
    // grows. Now the forward chain anchors on the most recent past month
    // with a recorded Historical Actual instead (lastActualUsage/
    // lastActualDfUsage below) — real, closed-month data — and falls back
    // to the live upload only when there's no past actual to anchor on at
    // all (e.g. a FIU's very first live month).
    let usage = NaN;
    let dfUsage = NaN;
    let usageForwardChain = NaN;
    let dfForwardChain = NaN;
    let lastActualUsage = NaN;
    let lastActualDfUsage = NaN;
    ...
    const monthly = monthCols.map((mc, i) => {
      ...
      if (mc.isPast) {
        ...
        const actualUsage = billingInfo && billingInfo.usageType === 'au' ? actualAu
          ...
          : NaN;
        // Remember the most recent past month's actual usage/DF-volume —
        // this is what the live compounding chain below anchors future
        // months on instead of the current month's live (possibly partial)
        // upload. Tracked independently for usage vs. dfUsage since a month
        // can have one without the other.
        if (!isNaN(actualUsage)) lastActualUsage = actualUsage;
        if (!isNaN(actualDf)) lastActualDfUsage = actualDf;
      ...
      // Scenario: Lending DF volume falls 50% (not the usual ~34%) during
      // the SUC Cliff only — see LENDING_CMGR_WORSE_RATE above for the
      // derivation. Only replaces the rate for a FIU whose own Cliff CMGR
      // is already negative (sucCliffCmgrRate < 0) — a FIU with a zero or
      // positive Cliff CMGR isn't degrading in the first place, so "worse"
      // has no sensible meaning for it and it's left on its own rate
      // (ask: 2026-09-03). SUC Recovery is deliberately left untouched
      // (recovery still happens at the originally-anticipated pace, not
      // accelerated). Pre-SUC months are untouched too (this whole branch
      // is inside sucActive).
      if (sucActive && sucPhase === 'cliff' && lendingCmgrWorseEligible && sucCliffCmgrRate < 0) rate = LENDING_CMGR_WORSE_RATE;
      ...
      // This month's own dfUsage is always the live upload's (possibly
      // MTD-extrapolated) figure, same as always; dfForwardChain — what
      // *future* months compound from — anchors on the last recorded
      // actual instead, when one exists (see the comment above
      // lastActualDfUsage).
      if (i === currentIndex) {
        dfUsage = dfBaselineUsage;
        dfForwardChain = !isNaN(lastActualDfUsage) ? lastActualDfUsage * (1 + rate) : dfBaselineUsage;
      } else if (!isNaN(dfForwardChain)) {
        dfForwardChain = dfForwardChain * (1 + rate);
        dfUsage = dfForwardChain;
      }
      // Every one-time adjustment below writes back into dfForwardChain too,
      // not just dfUsage (fixed 2026-09-10 — ask: "annual revenue ... way
      // higher than it should be" once a SUC Start Date was set). Splitting
      // dfUsage (this month's display value) from dfForwardChain (what
      // future months compound from) introduced a real regression here: a
      // one-time cut applied only to dfUsage was invisible to
      // dfForwardChain, so the *next* SUC month's compounding step
      // (dfForwardChain = dfForwardChain * (1 + rate), above) silently
      // resumed from the pre-cut figure — undoing a PFM/non-Bank FIU's /6
      // cut, or HDFC's 43% cut, one month after it was supposed to take
      // permanent effect, and letting Data-Fetch-driven SUC revenue run
      // roughly 6x (or 1/0.57x) too high from the second SUC month onward.
      if (sucActive && dfReductionEligible && !isNaN(dfUsage) && !dfReductionApplied) {
        dfUsage = dfUsage / 6;
        dfForwardChain = dfUsage;
      ...
      // License=Bank so that rule doesn't apply to it anyway). Re-applied
      // every SUC month regardless of what compounding did to
      // dfForwardChain in between, but kept in sync here too for
      // consistency (nothing downstream should ever see a stale
      // pre-override dfForwardChain for axisbank during SUC).
      ...
      // needed for months before SUC starts, and for FIUs SUC doesn't apply
      // to. Same split as the DF-volume chain just above: this month's own
      // usage is always the live upload; usageForwardChain (what future
      // months compound from) anchors on the last recorded actual instead,
      // when one exists.
      if (hasData) {
        if (i === currentIndex) {
          usage = baselineUsage;
          usageForwardChain = !isNaN(lastActualUsage) ? lastActualUsage * (1 + rate) : baselineUsage;
        } else if (!isNaN(usageForwardChain)) {
          usageForwardChain = usageForwardChain * (1 + rate);
          usage = usageForwardChain;
        }
      }
      ...
      // silent gap is what this closes. usageForwardChain/dfForwardChain
      // above are unaffected by this — they already anchor on the last
      // recorded actual usage/DF-volume, not on this month's revenue, so
      // next month keeps compounding off real usage either way; only the
      // *returned* figures for this month change. sucActive/sucPhase are
      // reported as-if pre-SUC (matching how a past actual is already
      // reported) since a recorded actual isn't a SUC-model output either
      // way.
  ...
  for (const c of counts) {
    ...
    // A second (or later) row for the same FIU ID in one upload — a real
    // risk in a hand-merged/re-exported file — used to get pushed as a
    // second row and double-counted into every total. Only the first
    // occurrence is used now; later ones are recorded in duplicateCounts
    // so the gap is visible instead of silently inflating revenue (fixed
    // 2026-09-03).
    if (seenKeys.has(key)) { duplicateCounts.push(fiuId); continue; }
    seenKeys.add(key);
    const meta = metadataById.get(key);
    const yc = yieldCmgrById.get(key);
    if (!meta) { unmatchedCounts.push(fiuId); continue; }
    rows.push(buildFiuRow(fiuId, key, meta, yc, c));
  }
  ...
  // A FIU with FIU Metadata config but no row in *this* month's counts
  // upload used to be dropped from the Annual Revenue table entirely —
  // silently, for every month including past ones with a real recorded
  // Historical Actual on file (e.g. a FIU that had zero activity this
  // cycle, or just wasn't in this particular export). Give it a row too:
  // blank counts (activeUsers/dataFetches: '') make baseline usage NaN, so
  // the live-projection months correctly come back "no data" rather than a
  // bogus zero-based projection, while past/current months still surface
  // whatever's recorded in Historical Actuals via the same `actual` lookup
  // every other row uses (fixed 2026-09-03 — see README's "Historical
  // actuals" section). unmatchedMeta above already lists exactly these FIU
  // IDs for the "N FIU(s) in your configs had no counts in this upload"
  // banner; this loop just also makes sure they aren't invisible.
  metadataById.forEach((meta, key) => {
    if (seenKeys.has(key)) return;
    const yc = yieldCmgrById.get(key);
    rows.push(buildFiuRow(meta.fiuId, key, meta, yc, { fiuId: meta.fiuId, activeUsers: '', dataFetches: '' }));
  });
  ...
  return { months: monthCols, currentIndex, rows, totalsByMonth, unmatchedCounts, duplicateCounts, unmatchedMeta, unbilledSummary, sucSummary };
}
